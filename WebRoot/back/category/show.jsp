...
<html>
	...
	<body style="background-color: #f0f9fd;text-align: center;">
		...
		<table bordercolor='#898E90' align='center' border='3px' cellpadding='10px' cellspacing="0px" >
			...
			<c:forEach items="${list}" var="c" varStatus="ccc">
				<tr>
					<td>${ccc.index+1 }</td>
					<td>${c.category_Id}</td>
					<td>${c.name}</td>
					...
					<td>${c.levels}</td>
					<td>
						<div class="button" onclick="location.href='${pageContext.request.contextPath}/category/deleteOne?id=${c.category_Id}'">删除 &raquo;</div>	
					</td>
				</tr>
			</c:forEach>
