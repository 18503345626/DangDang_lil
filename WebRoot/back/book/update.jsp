...
<body style="background-color: #f0f9fd;text-align: center">
	...
	<form action="${pageContext.request.contextPath}/book/updateOneBook" method="post" enctype="multipart/form-data">
			...
			<table>
					...
					<td>
						<select name="book.category_id" class="el-select__inner inner2">
							<c:forEach items="${listCategory}" var="category">
								<option <c:if test="${book.category_id==category.category_Id}">selected</c:if> value="${category.category_Id}">${category.name}</option>
							</c:forEach>
						</select>
					</td>
					<td>字数：</td>
				...
				<tr>
					<td>当当价：</td>
					<td><input type=number name="book.dprice" class="el-input__inner" value="${book.price}"></td>
					<td></td>
				</tr>
				<tr>
					<td>库存：</td>
